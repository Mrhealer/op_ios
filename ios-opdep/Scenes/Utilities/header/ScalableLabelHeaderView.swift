...
class ScalableLabelHeaderView: GSKStretchyHeaderView {
    ...
    private lazy var label: UILabel = {
        let label = UILabel(frame: CGRect(x: 0, y: 20, width: self.contentView.frame.width, height: self.contentView.frame.height - 20))
    ...
    private func commonInit() {
        self.maximumContentHeight = self.contentView.frame.width
