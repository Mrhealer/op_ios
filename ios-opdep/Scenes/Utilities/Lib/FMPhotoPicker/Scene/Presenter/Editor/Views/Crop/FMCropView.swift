...
class FMCropView: UIView {
    ...
    public func moveCroppedContentToCenterAnimated(duration: Double = kComplexAnimationDuration, complete: (() -> Void)? = nil) {
        guard cropBoxView.frame.width != 0, cropBoxView.frame.height != 0 else {
            return
        }
        var cropFrame = cropBoxView.frame
